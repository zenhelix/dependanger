...
public abstract class GenerateTomlTask : AbstractDependangerTask() {
    ...
    public val tomlFile: RegularFileProperty = project.objects.fileProperty().convention(
        extension.outputDirectory.file(extension.toml.filename)
    )
    ...
    public fun execute() {
        ...
        val tomlConfig = extension.toml.toConfig()
        val generator = TomlGenerator(tomlConfig)
        ...
        logger.lifecycle("Dependanger: Generated ${tomlConfig.filename} -> $outputFile")
    }
}
