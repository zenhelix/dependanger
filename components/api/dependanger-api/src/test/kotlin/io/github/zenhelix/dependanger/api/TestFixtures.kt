...
internal fun fakeLicenseCheck(
    ...
    FakeProcessor("fake-license-check", ProcessingPhase.LICENSE_CHECK, 130, LicenseViolationsExtensionKey, provider)

internal fun fakeTransitiveResolver(
    provider: (EffectiveMetadata) -> List<TransitiveTree>,
): FakeProcessor<List<TransitiveTree>> =
    FakeProcessor("fake-transitive", ProcessingPhase.TRANSITIVE_RESOLVER, 101, TransitivesExtensionKey, provider)

internal fun fakeConflictDetector(
    provider: (EffectiveMetadata) -> List<VersionConflict>,
): FakeProcessor<List<VersionConflict>> =
    FakeProcessor("fake-conflicts", ProcessingPhase.TRANSITIVE_RESOLVER, 102, VersionConflictsExtensionKey, provider)
