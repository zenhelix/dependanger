...
public class BomImportsDsl {
    ...
    public fun bom(alias: String, coordinates: String) {
        ...
        when (parts.size) {
            ...
            3 -> boms.add(BomImport(alias = alias, group = parts[0], artifact = parts[1], version = VersionReference.Literal(parts[2])))
    ...
    public fun bom(alias: String, coordinates: String, version: VersionReference) {
        ...
        boms.add(BomImport(alias = alias, group = parts[0], artifact = parts[1], version = version))
    }

    public fun bom(alias: String, group: String, artifact: String, version: VersionReference) {
        boms.add(BomImport(alias = alias, group = group, artifact = artifact, version = version))
    }
}
