...
- Все документы в формате AsciiDoc (`.adoc`)
- При создании новых документов всегда использовать соответствующий шаблон из `_templates/`
- Все диаграммы должны быть в формате Mermaid

### Диаграммы Mermaid

В AsciiDoc используй блок:
...
- `graph` / `flowchart` — блок-схемы и потоки
- `sequenceDiagram` — взаимодействие компонентов
- `classDiagram` — структура классов
- `stateDiagram-v2` — состояния и переходы
- `erDiagram` — модель данных
