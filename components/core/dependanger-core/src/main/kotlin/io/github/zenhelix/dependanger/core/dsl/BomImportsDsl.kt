...
public class BomImportsDsl {
    ...
    public fun bom(alias: String, coordinates: String) {
        ...
        when (parts.size) {
            ...
            3 -> _boms.add(BomImport(alias = alias, coordinate = MavenCoordinate(parts[0], parts[1]), version = VersionReference.Literal(parts[2])))
    ...
    public fun bom(alias: String, coordinates: String, version: VersionReference) {
        ...
        _boms.add(BomImport(alias = alias, coordinate = MavenCoordinate(parts[0], parts[1]), version = version))
    }

    public fun bom(alias: String, group: String, artifact: String, version: VersionReference) {
        requireUniqueAlias(alias, "BOM alias", _aliases)
        _boms.add(BomImport(alias = alias, coordinate = MavenCoordinate(group, artifact), version = version))
    }
}
